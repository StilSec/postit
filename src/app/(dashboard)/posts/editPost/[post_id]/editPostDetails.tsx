...
import { editPost } from "@/app/(dashboard)/posts/dataOps";
...
export default function EditPostDetailsPage(
    {postId, oldTitle, oldContent, oldSummary, oldThumbnail} :
 {postId: number, oldTitle: string, oldContent: string, oldSummary: string, oldThumbnail: string}) {
  const [title, setTitle] = useState(oldTitle);
  const [content, setContent] = useState(oldContent);
  const [summary, setSummary] = useState(oldSummary);
  ...
  const handleEdit = async () => {
    ...
    if(title.length < 1 || title.length > 50) {
      setErrorMessage("Title must be 1-50 characters long.")
      showErrorModal(true);
    }
    else {
  ...
  return (
    <div className="flex flex-col min-h-[80vh] space-y-5 sm:space-y-6 text-black">
      ...
      <Button type="button" size="sm" className="mt-3" onClick={handleEdit}>
        Update
      </Button>
      ...
      {/* Modal for verification of post editing */}
      {confirmModal ?
        (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog">
            <div 
                className="w-full max-w-[400px] rounded-xl bg-white shadow-2xl flex flex-col p-8 items-center text-center"
            ...
            <p className="text-slate-600 mb-8 text-sm">
                The post's information will be edited.
            </p>
